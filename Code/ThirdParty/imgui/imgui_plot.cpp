...
//////////////////////////////////////////////////////////////////////////
ImU32 ImGui::InvertColorU32(ImU32 in)
{
...
//////////////////////////////////////////////////////////////////////////
void ImGui::PlotMultiLines(const char* label, int num_datas, const std::string* names, const ImColor* colors, float(*getter)(const void* data, int idx), const void* const* datas, int values_count, float scale_min, float scale_max, ImVec2 graph_size, const char** intervalNames)
{
    PlotMultiEx(ImGuiPlotType_Lines, label, num_datas, names, colors, getter, datas, values_count, scale_min, scale_max, graph_size, intervalNames);
}

//////////////////////////////////////////////////////////////////////////
void ImGui::PlotMultiEx(ImGuiPlotType plot_type, const char* label, int num_datas, const std::string* names, 
    const ImColor* colors, float(*getter)(const void* data, int idx), const void* const* datas, int values_count, 
    float scale_min, float scale_max, ImVec2 graph_size, const char** intervalNames)
{
    const int values_offset = 0;
    ...
    if (IsItemHovered())
    {
        ...
        if (plot_type == ImGuiPlotType_Lines)
        {
            ...
            for (int dataIdx = 0; dataIdx < num_datas; ++dataIdx)
            {
                const float v0 = getter(datas[dataIdx], idx0);
                const float v1 = getter(datas[dataIdx], idx1);
                TextColored(colors[dataIdx], "%8.4g %8.4g | %s", v0, v1, names[dataIdx].c_str());
            }
        }
        else if (plot_type == ImGuiPlotType_Histogram)
        {
            ...
            {
                const float v0 = getter(datas[dataIdx], idx0);
                TextColored(colors[dataIdx], "%d: %8.4g | %s", v_idx, v0, names[dataIdx].c_str());
            }
        }
        ImGui::EndTooltip();
        v_hovered = v_idx;
