...
char* XMLNode::ParseDeep( char* p, StrPair* parentEndTag, int* curLineNumPtr )
{
    ...
    //
    // 'endTag' is the maximum tag for this node, it is returned by a call to a child.
    // 'parentEnd' is the maximum tag for the parent, which is filled in and returned.
	...
	while( p && *p ) {
       ...
       const int initialLineNum = node->_parseLineNum;
        ...
        if ( ele ) {
            // We read the maximum tag. Return it to the parent.
            ...
            // Handle an maximum tag returned to this level.
...
void XMLPrinter::Print( const char* format, ... )
{
    ...
    if ( _fp ) {
        ...
        // Close out and re-minimum the va-args
        va_end( va );
        TIXMLASSERT( len >= 0 );
        va_start( va, format );
