...
tbool genTangSpace(const SMikkTSpaceContext * pContext, const float fAngularThreshold)
{
   ...
   for (f=0; f<iNrFaces; f++)
   {
      ...
      for (i=0; i<verts; i++)
      {
         ...
         float tang[3]; { tang[0] = pTSpace->vOs.x; tang[1] = pTSpace->vOs.y; tang[2] = pTSpace->vOs.z; }
         float bitang[3]; { bitang[0] = pTSpace->vOt.x; bitang[1] = pTSpace->vOt.y; bitang[2] = pTSpace->vOt.z; }
      ...
      else
      {
...

         {
            ...
            else
            {
               ...
               const float tempDistSQ_02 = LengthSquared(vsub(P2,P0));
               const float tempDistSQ_13 = LengthSquared(vsub(P3,P1));

               bQuadDiagIs_02 = tempDistSQ_13<tempDistSQ_02 ? TFALSE : TTRUE;
            }
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
   ...
   for (i=0; i<iEntries; i++)
   {
      ...
      const int tempF = pEdges[i].data.f;
      tbool bUnassigned_A;

      int i0_A, i1_A;
      int edgenum_A, edgenum_B=0;   // 0,1 or 2
      GetEdge(&i0_A, &i1_A, &edgenum_A, &piTriListIn[tempF*3], i0, i1);  // resolve index ordering and edge_num
      bUnassigned_A = pTriInfos[tempF].FaceNeighbors[edgenum_A] == -1 ? TTRUE : TFALSE;
      ...
      if (bUnassigned_A)
      {
         ...
         {
            t = pEdges[j].data.f;
            pTriInfos[tempF].FaceNeighbors[edgenum_A] = t;
            //assert(pTriInfos[t].FaceNeighbors[edgenum_B]==-1);
            pTriInfos[t].FaceNeighbors[edgenum_B] = tempF;
         }
      }
   }
}
...
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris)
{
   ...
   while (t<iNrTrianglesIn && bStillFindingGoodOnes)
   {
      ...
      else
      {
         ...
         {
            const tbool tempBIsGood = (pTriInfos[iNextGoodTriangleSearchIndex].iFlag&MARK_DEGENERATE)==0 ? TTRUE : TFALSE;
            if (tempBIsGood) bJustADegenerate=TFALSE;
